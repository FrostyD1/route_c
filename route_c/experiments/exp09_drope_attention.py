...
class BaselineEnergy:
    """Baseline energy without D-RoPE (core + obs only)"""
    ...
    def energy_obs(self, z, o_obs, mask):
        with torch.no_grad():
            z_t = z.unsqueeze(0) if z.dim() == 3 else z
            z_t = z_t.to(self.device)
    ...
    def total_energy(self, z, o_obs, mask):
        ...
        return total, {'core': e_core, 'obs': e_obs, 'rope': 0.0, 'total': total}


class BaselineSolver:
    """Baseline solver without D-RoPE"""
    ...
    def run(self, z_init, o_obs, pixel_mask, bit_mask, n_sweeps=30, track_metrics=True, classifier_fn=None):
        ...
        metrics = {'energy': [], 'energy_breakdown': [], 'pred_label': [], 'pred_confidence': []}
...
def evaluate_occlusion(
    ...
    for idx in iterator:
        ...
        z_final, metrics = solver.run(
            ...
            n_sweeps=cfg.n_sweeps, track_metrics=True, classifier_fn=classifier_fn
        )
